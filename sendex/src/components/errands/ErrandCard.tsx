...
const ErrandCard: React.FC<Props> = ({ item, onClick }) => {

  return (
    <div role="button" tabIndex={0} onClick={() => onClick && onClick(item)} onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && onClick && onClick(item)} className="p-4 border rounded-md hover:shadow cursor-pointer bg-white dark:bg-slate-800">
